# from https://github.com/SaeedNB/DeepMDV
import argparse
import os
import numpy as np
import pickle
...
def generate_mdvrp_data(geodata_file: str, config):
    dataset = []
    for instance in range(config['dataset_size']):
        depot_latitudes = np.random.uniform(low=MIN_LAT, high=MAX_LAT, size=config['depot_size']).tolist()
        depot_longitudes = np.random.uniform(low=MIN_LON, high=MAX_LON, size=config['depot_size']).tolist()
        depots_coordinates = list(zip(depot_latitudes, depot_longitudes))

        node_latitudes = np.random.uniform(low=MIN_LAT, high=MAX_LAT, size=config['vrp_size']).tolist()
        node_longitudes = np.random.uniform(low=MIN_LON, high=MAX_LON, size=config['vrp_size']).tolist()
        ...
        instance = []
        instance.append(depots_coordinates)
        instance.append(nodes_coordinates)
        instance.append(demands)
        instance.append(config['capacity'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--capacity", help="Capacity of vehicles", default=50)
            ...
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsp':
                    ...
                    config = {
                        ...
                        'vrp_size': opts.vrp_size, 
                        'depot_size': opts.depot_size,
                        'capacity': opts.capacity
                    }

                    dataset = generate_mdvrp_data(geodata_file, config)
                elif problem == 'pctsp':
                ...
                print(f"DEBUG: First instance of dataset:\n")
                print(dataset[0])
